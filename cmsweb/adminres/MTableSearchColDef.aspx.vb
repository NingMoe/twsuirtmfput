...
Partial Class MTableSearchColDef
    Inherits CmsPage
...
#End Region
    ...
    Protected Overrides Sub CmsPageInitialize()
        ...
        Select Case VInt("PAGE_MTSLIST_TYPE")
            Case MTSearchType.MultiTableView
                ...
                '暂时允许所有用户编辑列表统计
                EnableSaveTitle(True)
                'Dim blnIsDepAdmin As Boolean = False
                'If OrgFactory.DepDriver.GetDepAdmin(CmsPass, CmsPass.GetDataRes(VLng("PAGE_RESID")).DepID) = CmsPass.EmpID Then
                '    blnIsDepAdmin = True
                'End If
                'EnableSaveTitle(CmsPass.EmpIsSysAdmin OrElse CmsPass.EmpIsAspAdmin OrElse blnIsDepAdmin)

                btnStartSearch.Enabled = True
                btnStartSearch.Attributes.Add("onClick", "return OpenMultiTableSearchResultWindow('" & VLng("PAGE_MTSHOSTID") & "')")

            Case MTSearchType.MultiTableViewForTemp
                lblTitle.Text = "临时列表统计"
                btnAddEmail.Visible = False
                btnAddMobile.Visible = False
            ...
            Case MTSearchType.RowRights
                lblTitle.Text = "行记录权限设置"
                btnAddEmail.Visible = False
                btnAddMobile.Visible = False
                ...
                Dim lngMTSHostID As Long = CmsDbBase.GetFieldLng(CmsPass, CmsTables.MTableHost, "MTS_ID", "MTS_TYPE=" & MTSearchType.RowRights & " AND MTS_RESID=" & VLng("PAGE_RESID") & " AND MTS_EMPID='" & VStr("PAGE_EMPID") & "'")
                If lngMTSHostID <> 0 Then
                    ViewState("PAGE_MTSHOSTID") = lngMTSHostID
                End If

            Case MTSearchType.AdvDictFilter
                lblTitle.Text = "高级字典行过滤设置"
                btnAddEmail.Visible = False
                btnAddMobile.Visible = False
                ...
                Dim lngMTSHostID As Long = CmsDbBase.GetFieldLng(CmsPass, CmsTables.MTableHost, "MTS_ID", "MTS_TYPE=" & MTSearchType.AdvDictFilter & " AND MTS_RESID=" & VLng("PAGE_RESID") & " AND MTS_DICTRESID=" & VLng("PAGE_ADVDICT_HOSTRES") & " AND MTS_COLNAME='" & VStr("PAGE_COLNAME") & "'")
                If lngMTSHostID <> 0 Then
                    ViewState("PAGE_MTSHOSTID") = lngMTSHostID
                End If

            Case MTSearchType.BatchSend
                '只有管理员才可以进入操作
                If CmsPass.EmpIsSysAdmin = False And CmsPass.EmpIsDepAdmin = False Then
                    Response.Redirect("/cmsweb/Logout.aspx", True)
                    Return
                End If
                lblTitle.Text = "批处理发送邮件短信"
                btnAddEmail.Visible = True
                btnAddMobile.Visible = True
                ...
                Case MTSearchType.ViewFilter
                    lblTitle.Text = "视图条件设置"
                    btnAddEmail.Visible = False
                    btnAddMobile.Visible = False
                    ...
                    Dim lngMTSHostID As Long = CmsDbBase.GetFieldLng(CmsPass, CmsTables.MTableHost, "MTS_ID", "MTS_TYPE=" & MTSearchType.ViewFilter & " AND MTS_RESID=" & VLng("PAGE_RESID") & " AND MTS_EMPID='" & VStr("PAGE_EMPID") & "'")
                    If lngMTSHostID <> 0 Then
                        ViewState("PAGE_MTSHOSTID") = lngMTSHostID
                    End If 
    ...
    Private Sub ShowAfterHostResSelected(ByVal lngHostResID As Long)
        If lngHostResID <> 0 Then
            ...
            If VInt("PAGE_MTSLIST_TYPE") = MTSearchType.GeneralRowWhere Or VInt("PAGE_MTSLIST_TYPE") = MTSearchType.PersonalRowWhere Or VInt("PAGE_MTSLIST_TYPE") = MTSearchType.RowRights Or VInt("PAGE_MTSLIST_TYPE") = MTSearchType.AdvDictFilter Then
