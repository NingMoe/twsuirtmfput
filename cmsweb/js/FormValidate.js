...
function CheckValue(frm){	
	...
	o=document.getElementById(frm).getElementsByTagName("input");
	for (i=0;i<o.length;i++){
		if (CheckValueType(o[i])==false) return false;
	}
	//判断所有的textarea框

	o=document.getElementById(frm).getElementsByTagName("textarea");
	for (i=0;i<o.length;i++)
	{
		if (CheckValueType(o[i])==false) return false;
	}
	...
	o=document.getElementById(frm).getElementsByTagName("select");
	for (i=0;i<o.length;i++){
...
{//递归获取父级DIV中包含‘Child’的DIV
//    var parentObj =obj.parentElement;
//    var id=parentObj.id;
//    if(id.indexOf("Child")>=0 && parentObj.nodeName=="DIV")
//    {
//        var index=id.substring(5);
//        ShowChild(index);
//    }
////    else if(parentObj.nodeName=="FORM")
////    {
////        $("tagContent").style.display="block";
////        $("Img_UpDown").src="../images/icon_2.gif";
////    }
//    else
//    {
//        GetShow(parentObj)
...
function CheckValueType(o){
	...
	if (o.mInput=="true"){
		...
		{
		    if(o.FieldType=="file"){
		       alert('请先选择要上传的'+o.FieldTitle);
	            GetShow(o);
	            o.focus();
		    }else
		    {
			alert(o.FieldTitle+'不能为空！');
			    if(o.FieldTitle!="品牌名称全称" && o.FieldTitle!="业态" && o.FieldTitle!="事件类型" && o.FieldTitle!="事件子类"){
	                GetShow(o);
	                o.focus();
	            }
	        }
	    ...
	    if(o.value!="")
			{
			    ...
			    {
			        alert(o.FieldTitle + "的格式不正确.ex:139********或021-58******或021-58******-分机号");
			        GetShow(o);
					o.focus();
					o.select();
					return false;
