...
document.write(".p{font-family:Calibri,Helvetica,sans-serif;font-size:12px;font-weight:normal;color:#1c4c75;}");
document.write("</style>");
...
{   //debugger;
    ...
    closeWindow1();    
    if(IsShield==true) CreateShieldBack(30);
...
//添加半透明层
function CreateShieldBack(shield)//ShieldDiv
{
...
//主调函数
function ShowDiv(content,center)
{
    document.body.style.overflow ="hidden";
    var divWidth="";
    var divHeight=0;
    if(parseInt(content.length)>0)
    {
        var clientWidth=document.documentElement.scrollWidth;
        var clientHeight=document.documentElement.scrollHeight;
        var pos;
      ...
      // alert("x:"+pos.x +";y:"+pos.y+";h:"+pos.h+";w:"+pos.w);
        
        
        var divHtml=document.createElement("div");
        divHtml.id="mesWindow1";
        divHtml.className="mesWindow";
        divHtml.innerHTML = divHtml.innerHTML + "<div class='mesWindowContent' id='mesWindowContent' style='background-color:#FFFFE1;'>"+content+"</div>";
        
        var style="left:"+(((clientWidth-pos.x-divWidth)<=0)?(pos.x-divWidth):pos.x)+"px;top:"+(((clientHeight-pos.y-(divHeight-20))<=0)?(pos.y-(divHeight-20)-(2*pos.h)):pos.y+(pos.h))+"px;position:absolute;width:"+divWidth+"px;";
        
        style += "border:1px solid #000000;font-size:12px;font-family: Calibri;" 
        divHtml.style.cssText=style;
        document.body.appendChild(divHtml);
    }
}
