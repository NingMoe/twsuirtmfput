...
/*打开系统帐户字典的选择窗体*/
function OpenSystemAccountWindow(ctlName, strUserName, strUserEnPass)
{
	var strUrl = '/cmsweb/adminres/FieldGetSystemAccount.aspx?ctlName='+ctlName+ "&user=" + escape(strUserName) + "&ucode=" + escape(strUserEnPass);
	OpenCenterWindow(strUrl,900,600,'sysaccount');
	return false;
}
/*打开定制编码的记录选择窗体*/
...
function OpenUploadFileWindow(strResID, strRecID)
{
   window.location.href="../cmsdocument/DocCheckin.aspx?mnuresid="+strResID+"&mnurecid="+strRecID + "&operation=upload&backpage=../cmshost/CmsFrmContent.aspx";
}
...
//删除子资源记录
function DeleteSubRecord(strResID,strNullMsg,strDelMsg){
    var ctlName = eval("self.document.forms(0).RECID3_" + strResID);
	if (ctlName.value == ""){
		alert(strNullMsg);
		return false;
	}
	if(window.confirm(strDelMsg))
	{
...
//提取子资源文档
function SubRecordGetDoc(strResID, strUserName, strUserEnPass,strMsg){
    var ctlName = eval("self.document.forms(0).RECID3_" + strResID);
	if (ctlName.value == ""){
		alert(strMsg);
		return false;
...
function OpenCenterWindow(url,width,height,winname){
	...
	var openWin = window.open(url,winname1,"top="+top+",left="+left+",height="+height+",width="+width+",scrollbars=yes,resizable=no,status=yes");
	try
	{
		openWin.focus();
	}
