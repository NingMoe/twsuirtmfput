<!--
//检查浏览器
DFlowTree.prototype.checkBrowser = function()
{
...
//设置TreeView的样式
DFlowTree.prototype.setStyle = function()
{
  /*
    width: 16px; \
    height: 16px; \
  ...
  */
  var style = "<style>"+
  "DIV.DFlowTree DIV IMG{border: 0px solid #FFFFFF;}"+
  "DIV.DFlowTree DIV SPAN IMG{border: 0px solid #FFFFFF;}";
  if(this.wordLine)
  {
    style +="\
    DIV.DFlowTree DIV\
...
//当根节点为空的时候做的处理
DFlowTree.prototype.atRootIsEmpty = function()
{
  ...
  for(var i=0; i<RCN.length; i++)
  {
    ...
    if (RCN[i].text=="")
    {
      ...
      {
        node.iconExpand  =  RCN[i].childNodes.length>1 ? HCN ? "PM0" : "lrb" : HCN ? "PM3" : "lr"
...
//初始化数据源里的数据以便后面的快速检索
DFlowTree.prototype.dataFormat = function()
{
  var a = new Array();
...
//在数据源检索所需的数据节点
//id  客户端节点对应的id
DFlowTree.prototype.load = function(id)
{
...
DFlowTree.prototype.nodeInit = function(sourceIndex, parentId)
{
  this.index++;
...
//name   要提取的属性名
DFlowTree.prototype.getAttribute = function(source, name)
{
...
DFlowTree.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  if(!isRoot)
  {
    ...
    {
      node.iconExpand  = HCN ? AtEnd ? "PM2" : "PM1" : AtEnd ? "ltr" : "ltrb";
...
DFlowTree.prototype.word2image = function(word)
{
  var str = "";
...
//id 树的客户端节点 id
DFlowTree.prototype.buildNode = function(id)
{
...
//聚集到客户端生成的某个节点上
//id  客户端树节点的id
DFlowTree.prototype.focusClientNode      = function(id)
{
...
//焦点聚集到树里的节点链接时的处理
//id 客户端节点 id
DFlowTree.prototype.focusLink= function(id)
{
...
//点击展开树节点的对应方法
DFlowTree.prototype.expand   = function(id, sureExpand)
{
...
//节点链接单击事件处理方法
//id 客户端树节点的 id
DFlowTree.prototype.nodeClick = function(id)
{
...
//sourceId 数据源里的节点 id
DFlowTree.prototype.getPath= function(sourceId)
{
...
//在源代码里指定 DFlowTree 初始聚集到某个节点
//sourceId 节点在数据源里的 id
DFlowTree.prototype.focus = function(sourceId, defer)
{
  if (!defer)
  {
...
//树的单击事件处理函数
DFlowTree.prototype.clickHandle = function(e)
{
...
//DFlowTree 双击事件的处理函数
DFlowTree.prototype.dblClickHandle = function(e)
{
  e = window.event || e; e = e.srcElement || e.target;
...
//回到树当前节点的父层节点
DFlowTree.prototype.upperNode = function()
{
...
//展开当前节点并
DFlowTree.prototype.lowerNode = function()
{
  if (!this.currentNode) this.currentNode = this.node["0"];
...
//聚集到树当前节点的上一节点
DFlowTree.prototype.pervNode = function()
{
...
//聚集到树当前节点的下一节点
DFlowTree.prototype.nextNode = function()
{
...
//展开树的所有节点
DFlowTree.prototype.expandAll = function()
{
...
//本树将要用动的图片的字义及预载函数
//path 图片存放的路径名
DFlowTree.prototype.setIconPath  = function(path)
{
...
//设置树的默认链接
//url 默认链接  若不设置, 其初始值为 #
DFlowTree.prototype.setURL     = function(url)
{
	this.url = url;
};
...
DFlowTree.prototype.setTarget = function(target)
{
	this.target = target;
};

//DFlowTree类，类初始化入口
function DFlowTree(strTreeName)
{
	if(typeof(strTreeName) != "string" || strTreeName == ""){
		throw(new Error('请输入有效的类名称。'));
	}
  
	//【property】
	this.url      = "#";
	this.target   = "_self";
	this.name     = strTreeName;
	this.wordLine = false;
  ...
  this.onkeydown= function(e)
  {
    ...
    switch(key)
    {
      case 37 : eval(strTreeName).upperNode(); break;  //Arrow left, shrink child node
      case 38 : eval(strTreeName).pervNode();  break;  //Arrow up
      case 39 : eval(strTreeName).lowerNode(); break;  //Arrow right, expand child node
      case 40 : eval(strTreeName).nextNode();  break;  //Arrow down
    }
  };
}
// -->
