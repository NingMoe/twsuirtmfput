...
Menu.prototype.cssFile = "/cmsweb/css/menu/officexp.css";
...
Menu.prototype.getMeasureDocument = function () {
	...
	if ( mf == null ) {
		...
		//Banks 2005-07-31 如果用appendChild会出现滚动条，因为这个iframe被插到当前页面的最后，
		//但当前页面必须有Panel1，稍后改进
		//document.body.appendChild( mf );
		Panel1.insertBefore(mf);
	}
...
Menu.prototype.show = function ( left, top, w, h ) {
	...
	//Banks_size 条目较少的菜单，height过高，减掉一点
	var tmpHeight = this.getPreferredHeight();
	if (tmpHeight < 65) tmpHeight = tmpHeight - 4; 
	if (tmpHeight > 65 && tmpHeight < 90) tmpHeight = tmpHeight - 2; 
	h = h || Math.min( window.screen.height, tmpHeight);
...
Menu.prototype.updateSizeCache = function ( bForce ) {
	...
	// preferred width
	cs.preferredWidth = d.body.scrollWidth + 20; //Banks_size: + 20
	//cs.preferredWidth = d.body.scrollWidth;

	// preferred height
	scrollContainer.style.overflow = "visible";
	cs.preferredHeight = body.firstChild.offsetHeight + 8; //body.scrollHeight; //Banks_size:  + 8
...
Menu.prototype.fixScrollButtons = function () {
	...
	if ( scrollContainer.scrollHeight > this.getHeight() ) {
		...
		this._scrollingMode = false; //banks 2005-08-26  true 改为 false
	}
	else {
