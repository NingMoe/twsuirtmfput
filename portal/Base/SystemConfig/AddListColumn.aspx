<%@ Page Language="C#" AutoEventWireup="true" CodeFile="AddListColumn.aspx.cs" Inherits="Base_SystemConfig_AddListColumn" %>

<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <title>资源列表显示字段配置</title>
 ...
 <script type="text/javascript" language="javascript">
            ...
            if (CheckValue("div<%=ResID %>FormTable")) {
                ...
                if (GetColName()) { 
                    $.ajax({
                        ...
                        dataType: "json",
                        data: { "Json": "" + jsonStr + "" },
                        url: "Ajax_Request.aspx?typeValue=SaveListInfo&ResID=<%=ResID %>&keyWordValue=<%=RelatedValue %>",
     ...
     function GetColName() {
         ...
         $("input[name='ChkColName']:checked").each(function () {
             var result = $("input[id='" + this.id.replace("Chk_", "txt_") + "']").val();
             if (result == "") {
                 ...
                 var jsonStr1 = "[{";
                 jsonStr1 += "'参数关键字':'<%=RelatedValue%>','显示字段':'" + result + "','绑定字段':'" + this.value + "','排序号':'" + sortNo + "','数据类型':'" + $("input[id='" + this.id.replace("Chk_", "txtDataType_") + "']").val() + "'";
                 jsonStr1 += "}]";
                 jsonStr += "," + jsonStr1;
    ...
    <form id="form1" runat="server">
        <div class="con" id="div<%=ResID %>FormTable" style="overflow-x: hidden; overflow-y:auto; position: relative; border: none;">
            ...
            <div title="列表显示字段配置" class="easyui-panel" style="overflow: hidden; padding: 5px; border-bottom: none; margin: 0px;"> 
