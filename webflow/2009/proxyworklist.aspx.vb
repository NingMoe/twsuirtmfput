...
Partial Class proxyworklist
    Inherits UserPageBase

    Protected arryProxyEmployees As ArrayList ' = ProxyTrans.GetProxyers(Currentuser.Code)
...
#End Region
    ...
    Private Sub BindData()
        ...
        Dim strValue As String = "FlowName LIKE '%{0}%' OR MAINFIELDVALUE LIKE '%{0}%' OR CREATORNAME LIKE '%{0}%' OR EMPNAME LIKE '%{0}%' OR CreateTime LIKE '%{0}%' "
        If Me.txtkeyvalue.Text.Trim() <> "" Then
            strCondition = String.Format(strValue, Me.txtkeyvalue.Text.Trim())
        End If

        DataGrid1.DataSource = Worklist.GetProxyWorklistItems(Currentuser.Code, strCondition)
        DataGrid1.DataBind()
