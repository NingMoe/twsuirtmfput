...
Public Class RecordEditBase
    ...
    Protected Function PageDealFirstRequest( _
        ...
        '必须在此处处理定制窗体信息
        'strFormName = FormManager.GetConditionFormName(pst, lngResID, lngMode, strFormName, hashFieldRelVal, lngRecID)
        ViewState("PAGE_FORMNAME") = strFormName
        'Dim strUrl As String = CTableFormCustomize.GetCustFormUrl(pst, lngResID, strFormName, lngMode)
        'If strUrl <> "" Then Throw New CmsExceptionCustForm(strUrl)
    ...
    Private Sub CommandSaveRecord( _
        ...
        Try
            ...
            If lngMode = InputMode.AddInHostTable Then
                ...
                If Not lblStatus Is Nothing Then lblStatus.Text = "－" & CmsMessage.GetUI(pst, "TITLE_RECORD_STATUS_EDIT")
        ...
        If strErrMsg <> "" Then PromptMsg(strErrMsg, True, Nothing)
    End Sub
    ...
    Protected Sub SetButtonStatus( _
        ...
        If lngMode = InputMode.AddInHostTable OrElse lngMode = InputMode.AddInRelTable Then
            If Not lblHeaderAction1 Is Nothing Then lblHeaderAction1.Text = "－" & CmsMessage.GetUI(pst, "TITLE_RECORD_STATUS_ADD")
        ...
        ElseIf lngMode = InputMode.EditInHostTable OrElse lngMode = InputMode.EditInRelTable Then
            If Not lblHeaderAction1 Is Nothing Then lblHeaderAction1.Text = "－" & CmsMessage.GetUI(pst, "TITLE_RECORD_STATUS_EDIT")
            If Not lnkSave Is Nothing Then lnkSave.Enabled = True
        ElseIf lngMode = InputMode.ViewInHostTable OrElse lngMode = InputMode.ViewInRelTable Then
            If Not lblHeaderAction1 Is Nothing Then lblHeaderAction1.Text = "－" & CmsMessage.GetUI(pst, "TITLE_RECORD_STATUS_VIEW")
    ...
    '        Return ResLocation.HostTable
    '    Else
    '        Return ResLocation.RelTable
    '    End If
    'End Function
    ...
    '保存记录
    '--------------------------------------------------------------------------
    Protected Sub SaveRecordOnUI(ByRef pst As CmsPassport, ByRef Panel1 As System.Web.UI.WebControls.Panel, ByRef lblHeaderAction1 As System.Web.UI.WebControls.Label, ByRef lnkSave As System.Web.UI.WebControls.LinkButton, ByVal blnShowSaveButton As Boolean, ByRef forceRights As ForceRightsInForm)
        ...
        Try
            ...
            If pst.GetDataRes(VLng("PAGE_RESID")).ResTableType = "DOC" And blnHasDocUploaded = False And (CType(VLng("PAGE_INMODE"), InputMode) = InputMode.AddInHostTable OrElse CType(VLng("PAGE_INMODE"), InputMode) = InputMode.AddInRelTable) Then
                strPromptMsg = CmsMessage.GetMsg(pst, "TIP_ADDDOC_LATER")
            End If
