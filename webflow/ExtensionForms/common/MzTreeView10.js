...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  var ObjectID = this.getAttribute(source, "id");
  if(!url) url = this.url;
  ...
  //add by chenyu 2009-8-9 
  if (this.getAttribute(source, "check")=="true")
  {
	var checkname=this.getAttribute(source, "check-name");
	if (checkname=="") checkname = "checktree";
	var checked=this.getAttribute(source, "checked");
	var checkvalue=this.getAttribute(source, "check-value");
	if (checkvalue=="") checkvalue = ObjectID;
	
	HTML += "<input type='checkbox' value='"+checkvalue+"' name='"+checkname+"'";
  ...
  HTML += "<IMG "+
    ...
    "onclick=\"return "+ this.name +".nodeClick('"+ id +"','"+ ObjectID +"')\">"+ node.text +
  "</A></NOBR></DIV>";
...
//节点链接单击事件处理方法
//id 客户端树节点的 id
//ObjectID节点数据的唯一ID (陶琼添加，【在加载tree是需要指定id的值 例:(tree.nodes['1_2'] = 'text:node 2; id:9000';)，其将返回‘9000’】目的：点击节点时返回数据的唯一ID。)
MzTreeView.prototype.nodeClick = function(id,ObjectID)
{
...
MzTreeView.prototype.setIconPath  = function(path)
{
  ...
  for(var i in this.icons)
  {
    ...
    //chenyu 取消用文字显示树线条的功能. 2008-12-4
    //if(k==9 && (new Date().getTime()-d)>20) this.wordLine = true; //(chenyu 取消本行. 2008-12-4)
