...
//debugger;
	...
	var i;
	//判断所有的text框
	o=frm.getElementsByTagName("input");
	
	for (i=0;i<o.length;i++){ 
		if (CheckValueType(o[i])==false) return false;
	}
	//判断所有的textarea框
	o=frm.getElementsByTagName("textarea");
	for (i=0;i<o.length;i++)
	{
		if (CheckValueType(o[i])==false) return false;
	}
	//判断所有的select框
	o=frm.getElementsByTagName("select");
	for (i=0;i<o.length;i++){
...
function CheckValueType(o){	
	var errmsg=""
	if(o.attributes["errmsg"]!=undefined) errmsg=o.attributes["errmsg"].value;
				
	//检查是否为必输项;
	if(o.attributes["mInput"]!=undefined)
	{
		if (o.attributes["mInput"].value=="true"){
			if (o.value.trim()=="")
			{
				alert(errmsg+'不能为空！');
				o.focus();
	...
	//类型判断;
	if(o.attributes["fType"]!=undefined)
	{//alert(o.attributes["fType"].value);
		switch (o.attributes["fType"].value)
		{
			case "num":
				if (isNaN(o.value))
				{
